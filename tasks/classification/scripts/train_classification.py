...
def _maybe_build_class_to_idx(tonic_dataset, ds_cfg):
    if not ds_cfg.get("needs_label_mapping", False):
        return None
    return _build_class_to_idx(tonic_dataset)
...
def _limit_indices(indices, limit):
    if limit is None:
        return list(indices)
    return list(indices)[: min(int(limit), len(indices))]
...
def build_dataloaders(args, representation, ds_cfg):
    ...
    if ds_cfg["official_split"]:
        ...
        train_indices = _limit_indices(range(len(train_tonic)), args.train_limit)
        test_indices = _limit_indices(range(len(test_tonic)), args.test_limit)
        train_set   = Subset(EventDataset(train_tonic, representation), train_indices)
        test_set    = Subset(EventDataset(test_tonic,  representation), test_indices)
    else:
        all_tonic    = tonic_cls(save_to=args.data_root)
        class_to_idx = _maybe_build_class_to_idx(all_tonic, ds_cfg)
        ...
        train_indices = _limit_indices(train_indices, args.train_limit)
        test_indices = _limit_indices(test_indices, args.test_limit)
...
def main():
    ...
    parser.add_argument("--dataset",        default="ncaltech101",
                        help="Dataset name. Also accepts aliases such as cifa, cifar, cifar10-dvs, n-mnist.")
    ...
    parser.add_argument("--device",       default=None,
                        help="Training device. Defaults to cuda when available, otherwise cpu.")
    parser.add_argument("--seed",         type=int,   default=42)
    parser.add_argument("--max_events",   type=int,   default=50000)
    parser.add_argument("--train_limit",  type=int,   default=None,
                        help="Optional cap on train samples for smoke tests.")
